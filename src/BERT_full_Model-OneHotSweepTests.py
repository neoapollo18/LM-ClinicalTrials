...
def train():
    ...
    class CombinedModel(nn.Module):
        def __init__(self, num_extra_features, num_classes, dropout_rate):
    ...
    test_filepaths = ['TrialData/.csv', 'TrialData/test2.csv', ...]
    test_dfs = [pd.read_csv(filepath) for filepath in test_filepaths]
    test_data_list = []
    ...
    test_data_list.append(test_dataloader)
    ...
    for epoch_i in range(0, epochs):
        ...
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            ...
            logits, attentions = combined_model(b_input_ids, b_input_mask, b_features)
        ...
        all_attentions = []  # Store attention weights for all inputs
        ...
        for batch in test_dataloader:
            ...
            all_attentions.extend(attentions[-1])  # Store the attention weights from the last layer

        # After testing, visualize attention weights for a few specific inputs
        for i in range(10):  # Visualize for the first 3 inputs as an example
            sample_input = test_inputs[i].unsqueeze(0).to(device)
            attention_weights = all_attentions[i]  # get the attention weights for the ith input

            # Convert input IDs back to tokens for visualization
            input_tokens = tokenizer.convert_ids_to_tokens(sample_input[0].tolist())
            plot_attention(attention_weights, input_tokens)
