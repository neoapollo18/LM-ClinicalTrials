...
df = pd.read_csv('ClinicalTrials/src/trial_data1.csv')
...
df = pd.read_csv('TrialData/2016_to_present.csv')
...
class CombinedModel(nn.Module):
    def __init__(self, num_extra_features, num_classes, dropout_rate=0.6):
        super(CombinedModel, self).__init__()
    ...
    def forward(self, input_ids, attention_mask, extra_features):
        ...
        return x
>>>>>>> 8f853ce71aa352cea5952e9743977573ac61e35e
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
combined_model = CombinedModel(num_extra_features=train_onehot_encoded.shape[1], num_classes=2, dropout_rate=0.6)
combined_model = combined_model.to(device)

optimizer = AdamW(combined_model.parameters(), lr = 0.0005,)
